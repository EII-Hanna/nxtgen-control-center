  ...
  function render() {
    ...
    el.innerHTML = `<div class="ui-page automation-page">
      ...
      <article class="ui-card automation-workflow-card">
        ...
        <div class="automation-config-grid"><div><label>WEBHOOK-URL</label><div class="automation-copy"><input readonly id="leadWebhookUrl" value="${esc(url)}"><button class="ui-btn ghost" data-copy="#leadWebhookUrl">Kopieren</button></div></div><div><label>STATUS</label><button class="ui-btn ${connection?.status==='active'?'ghost':'primary'}" id="toggleLeadWorkflow">${connection?.status==='active'?'Workflow pausieren':'Workflow aktivieren'}</button></div></div>
        ${connection?.last_error?`<div class="automation-error"><b>Letzter Fehler</b><span>${esc(connection.last_error)}</span></div>`:''}
      </article>
      <div class="automation-layout">
        <article class="ui-card"><h3>Workflow testen</h3><p class="automation-muted">Dieser Test verwendet denselben Webhook wie ein echtes Website-Formular.</p><form id="automationTestForm" class="automation-form"><label>Unternehmen<input name="company_name" required placeholder="Muster GmbH"></label><div class="automation-two"><label>Ansprechpartner<input name="contact_name" placeholder="Max Mustermann"></label><label>E-Mail<input name="email" type="email" placeholder="max@muster.de"></label></div><label>Bedarf / Nachricht<textarea name="message" placeholder="Wir möchten unsere Leadbearbeitung automatisieren."></textarea></label><button class="ui-btn primary" type="submit">Test-Lead senden</button></form></article>
...

{
  ...
  function boot() { installNavigation(); ensureView(); }
