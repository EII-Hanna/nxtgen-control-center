-- NXTGEN payment automation: provider webhook -> invoice -> activation gate
...
declare
  ...
  v_is_paid boolean;
begin
  v_is_paid := lower(coalesce(p_event_type,'')) in (
  ...
  if v_invoice.id is null and nullif(p_payload->>'source_package_id','') is not null then
    select i.* into v_invoice
    from public.backoffice_invoices i
    where i.metadata->>'source_package_id'=p_payload->>'source_package_id'
  ...
  update public.backoffice_payment_webhooks set organization_id=v_invoice.organization_id,invoice_id=v_invoice.id,contract_id=v_invoice.contract_id
  where id=v_webhook_id;
  ...
  perform public.mark_backoffice_invoice_paid(v_invoice.id,coalesce(p_amount,v_invoice.total_amount-v_invoice.paid_amount),lower(p_provider),p_provider_reference);
  ...
  if v_contract.contract_package_id is not null then
    update public.contract_packages set payment_status='paid',updated_at=now() where id=v_contract.contract_package_id;
  end if;

  insert into public.delivery_notifications(organization_id,channel,event_type,severity,message,metadata)
  values(v_invoice.organization_id,'in_app','finance.payment.received','info',
    'Zahlung eingegangen · Rechnung '||v_invoice.invoice_number||' wurde über '||lower(p_provider)||' bezahlt.',
  ...
  return jsonb_build_object('status','processed','invoice_id',v_invoice.id,'invoice_number',v_invoice.invoice_number,
    'activation_gate_id',v_gate.id,'activation_status',v_gate.status,'webhook_id',v_webhook_id);
...
begin
  ...
  select * into v_workspace from public.client_workspaces where client_id=v_gate.client_id and organization_id=v_gate.organization_id limit 1;
  ...
  update public.client_repo_sections set status='active',updated_at=now() where workspace_id=v_workspace.id and section_key='onboarding';
