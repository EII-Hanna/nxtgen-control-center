-- NXTGEN stable automation hub: lead intake plus provider-neutral meeting intelligence
...
create policy automation_connections_org_access on public.automation_connections for all using (public.is_org_member(organization_id)) with check (public.is_org_member(organization_id));
drop policy if exists automation_run_log_org_access on public.automation_run_log;
create policy automation_run_log_org_access on public.automation_run_log for select using (public.is_org_member(organization_id));
drop policy if exists meeting_records_org_access on public.meeting_records;
create policy meeting_records_org_access on public.meeting_records for all using (public.is_org_member(organization_id)) with check (public.is_org_member(organization_id));

create index if not exists idx_automation_connections_org on public.automation_connections(organization_id,status);
create index if not exists idx_automation_run_log_org_created on public.automation_run_log(organization_id,started_at desc);
...
select o.id,'lead-intake','Lead Intake Webhook','webhook','active' from public.organizations o where o.type='internal'
on conflict(organization_id,connection_key) do nothing;
insert into public.automation_connections(organization_id,connection_key,name,provider,status)
select o.id,'meeting-intake','Zoom / Meeting Webhook','zoom','active' from public.organizations o where o.type='internal'
on conflict(organization_id,connection_key) do nothing;

create or replace function public.ensure_lead_intake_connection()
returns public.automation_connections language plpgsql security definer set search_path=public as $$
